...
void text6x8b4(uint24_t xW, uint24_t yW, uint8_t lexicon) { //Incompatibile with 0.70.Alpha and later
    ...
    uint8_t* fill0 = (uint8_t*)VRAM + (yW * 320 + xW);
...
void fadeIn() {
    uint16_t *fP = paletteRAM; //Fade in after setup
...
void winScreen() {
    ...
    *frameBase = (uint24_t)((size_t)WRAM);
    *videoMode = 0x925; //4bit mode
    gColor = 0;
    fillRect(0,240,320,120); //Clears the screen
