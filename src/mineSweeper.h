...
void displaySeg7(uint24_t i, uint8_t b, struct dataSeg7* lcd) { //Number, Digits, X-2, Y-2
    ...
    uint24_t pow = 1;
    uint24_t z = (uint24_t)((size_t)VRAM) + y * 320; //Constant offset
    x += ((d << 3) + d) - 9; //d * 9 - 9 == 9 * (d - 1)
    for (uint8_t n = 0; n < d; n++) { //Since d was decremented
...
void flagDraw(uint8_t symbol, uint24_t xP, uint24_t yP) {
    ...
    uint8_t* z = (uint8_t*)VRAM + (yP * 320 + xP);
...
void glyph(int16_t space, uint8_t symbol) {
    ...
    if (font == 2) { //6x8 Font
        ...
        uint8_t* z = (uint8_t*)VRAM + ((space / marX * disY + posY + padY) * 320 + (space % marX * disX + posX + padX));
        uint8_t b = 1;
...
void cursorSetup() {
    ...
    *LCDTiming2 = (uint32_t)(*LCDTiming2 & ~(uint32_t)0x03FF0000) | (uint32_t)(320 - 1) << 16; //Set CPL
}
